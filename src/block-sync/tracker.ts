...
export class BlockChangeTracker {
  private previousSnapshot: Map<BlockId, { block: Block; index: number }> = new Map();
  ...
  private takeSnapshot(): void {
    ...
    this.currentBlocks.forEach((block, index) => {
      this.previousSnapshot.set(block.clientId, { block: { ...block }, index });
    });
  }
  ...
  updateFromEditor(editorBlocks: Block[]): BlockOperation[] {
    ...
    const newBlocksMap = new Map<BlockId, { block: Block; index: number }>();
    ...
    newBlocksMap.forEach(({ block, index }, clientId) => {
      ...
      if (!previous) {
        ...
        if (JSON.stringify(previous.block) !== JSON.stringify(block)) {
          operations.push({
            type: 'update',
            index,
            block,
  ...
  markOperationsSynced(operations: BlockOperation[]): void {
    const syncedTimestamps = new Set(operations.map(op => op.timestamp));
