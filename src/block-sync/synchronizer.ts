import { Block, BlockOperation, TrackedBlock } from './types';
...
export class BlockSynchronizer {
		...
		operations.forEach( ( op ) => {
			switch ( op.type ) {
				case 'insert': {
					...
					const insertedBlock = this.localBlocks.find(
						( b ) => b.clientId === op.block.clientId
					);
					if ( insertedBlock ) {
				...
				case 'update': {
					const updatedBlock = this.localBlocks.find(
						( b ) => b.clientId === op.block.clientId
					);
					if ( updatedBlock ) {
				...
				case 'delete': {
					...
					const deletedBlock = this.localBlocks.find(
						( b ) => b.clientId === op.block.clientId
					);
					if ( deletedBlock ) {
	...
	applyRemoteOperations( remoteOps: BlockOperation[] ): {
		...
		const sortedOps = [ ...remoteOps ].sort(
		...
		sortedOps.forEach( ( op ) => {
			switch ( op.type ) {
				case 'insert': {
					...
					const existingLocal = this.localBlocks.find(
						( b ) => b.clientId === op.block.clientId
					);
				...
				case 'update': {
					const localBlock = this.localBlocks.find(
						( b ) => b.clientId === op.block.clientId
					);

					if ( localBlock ) {
						if (
							localBlock.locallyModified &&
							! localBlock.pendingDelete
						) {
				...
				case 'delete': {
					const blockToDelete = this.localBlocks.findIndex(
						( b ) => b.clientId === op.block.clientId
					);
					...
					if ( blockToDelete !== -1 ) {
						...
						if ( block.locallyModified && ! block.pendingDelete ) {
							...
							conflicts.push( {
								...
								resolution:
									this.conflictStrategy === 'remote-wins'
										? ( {
												...op.block,
												pendingDelete: true,
										  } as Block )
				...
				case 'move': {
					const blockIndex = this.localBlocks.findIndex(
						( b ) => b.clientId === op.block.clientId
					);
					if ( blockIndex !== -1 && blockIndex !== op.toIndex ) {
						const [ block ] = this.localBlocks.splice(
							blockIndex,
							1
						);
		...
		this.localBlocks = this.localBlocks.filter(
			( b ) => ! b.pendingDelete
		);
